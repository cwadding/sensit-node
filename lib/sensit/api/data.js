...
Data.prototype.find = function (options, callback) {
  ...
  this.client.get('/api/topics/' + this.topic_id + '/feeds/' + this.feed_id + '/data/' + this.id + '', body, options, function(err, response) {
    if (err) {
      return callback(err);
    }
...
Data.prototype.update = function (options, callback) {
  ...
  this.client.put('/api/topics/' + this.topic_id + '/feeds/' + this.feed_id + '/data/' + this.id + '', body, options, function(err, response) {
    if (err) {
      return callback(err);
    }
